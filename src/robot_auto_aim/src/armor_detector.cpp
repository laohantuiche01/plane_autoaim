...
Detector::Detector(const int &bin_thres,
                   const robot_utils::EnemyColor &color,
                   const LightParams &l,
                   const ArmorParams &a)
  ...
  for (const auto &contour : contours) {
    ...
    if (isLight(light)) {
      ...
      if (std::abs(sum_r - sum_b) / static_cast<int>(contour.size()) >
        ...
        light.color = sum_r > sum_b ? robot_utils::EnemyColor::RED : robot_utils::EnemyColor::BLUE;
      }
...
bool Detector::isLight(const Light &light) noexcept {
  ...
  robot_interfaces::msg::DebugLight light_data;
...
ArmorType Detector::isArmor(const Light &light_1, const Light &light_2) noexcept {
  ...
  // Fill in debug information
  robot_interfaces::msg::DebugArmor armor_data;
...
void Detector::drawResults(cv::Mat &img) const noexcept {
  ...
  //   auto line_color =
  //     light.color == robot_utils::EnemyColor::RED ? cv::Scalar(0, 255, 255) : cv::Scalar(255, 255, 0);
