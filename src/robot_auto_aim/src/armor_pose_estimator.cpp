...
namespace robot_auto_aim
{
    ...
    {
        // Setup pnp solver
        pnp_solver_ = std::make_unique<robot_utils::PnPSolver>(camera_info->k, camera_info->d);
                                          ...
                                          Eigen::Matrix3d R_imu_camera)
    {
        ...
        for (const auto& armor : armors)
        {
                ...
                (armor.type == ArmorType::SMALL ? "small" : "large")))
            {
                ...
                Eigen::Matrix3d R = robot_utils::cvToEigen(rmat);
                ...
                robot_interfaces::msg::Armor armor_msg;
                                           ...
                                           std::vector<cv::Mat>& tvecs) const
    {
        ...
        // 转换为Eigen矩阵
        Eigen::Matrix3d R1 = robot_utils::cvToEigen(R1_cv);
