...
namespace ms {
    ...
    private:
        ...
        bool scan() {
            ...
            for (int i = 0; (size_t) i <= (size_t) rxBuffer.size() - sizeof(Head) - sizeof(Tail); i++) {
                ...
                int headCheckerResult = 0;
                for (const auto& checker: headCheckers) {
                    headCheckerResult = checker(head);
                    if (headCheckerResult) {
                        if (errorHandle) {
                            errorHandle(headCheckerResult, rxBuffer.substr(i, sizeof(Head)));
                        }
                        break;
                    }
                }
                ...
                if ((size_t) size < length + sizeof(Tail) + sizeof(Head)) {
                    okHeadFound = true;
                    continue;
                }
                ...
                int tailCheckerResult = 0;
                for (const auto& checker: tailCheckers) {
                    tailCheckerResult = checker(tail, p, sizeof(Head) + length);
                    if (tailCheckerResult) {
                        if (errorHandle) {
                            errorHandle(tailCheckerResult, rxBuffer.substr(i, sizeof(Head) + length + sizeof(Tail)));
                        }
                        break;
                    }
                }
                ...
                if (cRes && errorHandle) {
                    errorHandle(cRes - 1, rxBuffer.substr(i, sizeof(Head) + length + sizeof(Tail)));
                }
                eraseSize = i + sizeof(Head) + length + sizeof(Tail);
                frameFound = true;
                break;
            }
            if (eraseSize > 0) {
            ...
            if (frameFound && rxBuffer.size() >= sizeof(Head) + sizeof(Tail)) {
                scan();
            }
            return frameFound;
        }
    ...
    public:
        ...
        bool push(const char *buffer, int len) {
            ...
            if (rxBuffer.size() > (size_t)maxSize) {
                rxBuffer.erase(0, rxBuffer.size() - maxSize);
            }
            return scan();
        }
